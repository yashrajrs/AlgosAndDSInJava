...
public class RemoveInvalidParenthesis {
    ...
    public void dfs(String left, String right, int countLeft, int maxLeft){
        ...
        if (left.charAt(0) =='('){
            ...
            dfs(left.substring(1), right, countLeft, maxLeft); // keep
        } else if (left.charAt(0) == ')') {
            if (countLeft>0) {
                dfs(left.substring(1), right + ")", countLeft-1, maxLeft); // keep
            }
            dfs(left.substring(1), right, countLeft, maxLeft); // keep
        } else {
            dfs(left.substring(1), right + String.valueOf(left.charAt(0)), countLeft, maxLeft); // keep
        }
    }
